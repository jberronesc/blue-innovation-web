...
import BudgetPartidaIncomeSearch from "./components/Search"
import BudgetPartidaIncomeTable from "./components/Table"
...
export default async function Page({
  ...
  const [regitersRes] = await Promise.all([
    await new FetchBackGETTokenBlueI({
      url: "/budget/v1/budget-partida-incomes/list",
      shearhParams: {
        page: page.toString(),
        ...(query && { query }),
  ...
  return (
    <ListBase
      ...
      title="Partidas presupuestarias de ingresos"
      info={<BudgetPartidaIncomeSearch provokeBack={provokeBack} />}
      pagination={
        <Pagination
    ...
    >
      <BudgetPartidaIncomeTable registers={registersData.results} />
    </ListBase>
  )
}
