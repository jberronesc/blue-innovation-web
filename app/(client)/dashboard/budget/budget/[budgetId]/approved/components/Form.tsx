...
export default function BudgetApprovedForm({
  registerToEdit,
}: {
  ...
  const {
    ...
    handleSubmit,
    getValues,
  } = useForm<BudgetApprovedType>({
    resolver: zodResolver(BudgetApprovedSchema),
  })
    ...
    onSuccess: async () => {
      ...
      return (
        await new FetchPATCHTokenBlueI({
          url: `/budget/v1/budgets/${registerToEdit.id}/approved`,
          body: {
            ...data,
          },
        }).execWithoutResponse()
